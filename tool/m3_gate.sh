#!/usr/bin/env bash
# M3 acceptance gate — buffer-mobile
#
# Spec refs: FR-03, FR-08, FR-17, FR-19, EC-26, R-03, MC-01, OQ-11,
#            NFR-04 (domain purity), NFR-07 (on-device convergence)
...
# Mirrors the structure and style of tool/m2_gate.sh.
#
# Usage:
#   bash tool/m3_gate.sh          # from the project root
#   ./tool/m3_gate.sh
...
# integration test (integration_test/list_continuation_test.dart) is tagged
...
#
# Verifies the M3 composition root (ProviderScope + BufferApp + BufferScreen +
# all M3 providers) mounts without exception using the flutter-tester VM device.
...
# ──────────────────────────────────────────────────────────────────────────────
# Note on the on-device list-continuation integration test (NFR-07):
#
# integration_test/list_continuation_test.dart is tagged @Tags(['on-device'])
...
#       --device-id <device-id>
#
# This test is the M3 phase-transition gate (R-03/MC-01/NFR-07): it asserts
# that a real soft-keyboard Return on "- item" produces "- item\n- " with the
# caret after the inserted token — convergence with the hardware-Enter widget
