...
void main() {
  ...
  // ---------------------------------------------------------------------------
  // TASK-04 (sp-20260615) — SettingsNotifier.setShowLineNumbers
  // Spec refs: FR-13, NFR-05
  // Contract C6 (§5.1): Future<void> setShowLineNumbers(bool enabled)
  //
  // Verifies:
  //   a. happy — from false (default), setShowLineNumbers(true) → state
  //      showLineNumbers == true; repo.save called once with showLineNumbers:true.
  //   b. toggle back — from true, setShowLineNumbers(false) → false; repo.save
  //      once with showLineNumbers:false.
  //   c. EC-07 no-op guard — from false, setShowLineNumbers(false) → repo.save
  //      NOT called.
  //   d. EC-08 AsyncLoading fallback — state AsyncLoading → reads
  //      `?? const AppSettings()` (showLineNumbers false), writes true, repo.save
  //      once, no throw.
  //   e. Optimistic update — state is AsyncData(showLineNumbers:true) synchronously
  //      BEFORE repo.save resolves.
  //   f. NFR-05 no new field — AppSettings field/prop count unchanged (the
  //      showLineNumbers field and kShowLineNumbers key already existed before
  //      this task; only the notifier verb is new).
  // ---------------------------------------------------------------------------

  group('SettingsNotifier.setShowLineNumbers — FR-13 / NFR-05 / C6', () {
    test(
      'given_showLineNumbers_false_when_setShowLineNumbers_true_then_state_is_true_and_save_called_once_with_true',
      () async {
        // Arrange: default showLineNumbers == false.
        final repo = _RecordingRepository(initial: const AppSettings());
        final container = ProviderContainer(
          overrides: [settingsRepositoryProvider.overrideWithValue(repo)],
        );
        ...
        // Settle the provider so state is AsyncData with showLineNumbers false.
        await container.read(settingsProvider.future);
        ...
        expect(
          ...
          equals(1),
          reason:
              'FR-13 C6: save must be called exactly once when showLineNumbers differs',
        );
        expect(
          repo.savedArgs.first.showLineNumbers,
          isTrue,
          reason: 'FR-13 C6: save must be called with showLineNumbers: true',
        );
        ...
        expect(
          state,
          isA<AsyncData<AppSettings>>(),
          reason: 'FR-13 C6: state must be AsyncData after optimistic update',
        );
        expect(
          state.value?.showLineNumbers,
          isTrue,
          reason:
              'FR-13 C6: optimistic state must reflect showLineNumbers: true',
        );
      },
    );

    test(
      'given_showLineNumbers_true_when_setShowLineNumbers_false_then_state_is_false_and_save_called_once_with_false',
      () async {
        // Arrange: start with showLineNumbers == true.
        final repo = _RecordingRepository(
        ...
        // Assert: save called exactly once with false.
        expect(
          repo.savedArgs.length,
          equals(1),
          reason:
              'FR-13 C6: save must be called exactly once when toggling back to false',
        );
        expect(
          repo.savedArgs.first.showLineNumbers,
          isFalse,
          reason: 'FR-13 C6: save must be called with showLineNumbers: false',
        );

        // Assert: state reflects the toggled value.
        final state = container.read(settingsProvider);
        expect(
          state.value?.showLineNumbers,
          isFalse,
          reason:
              'FR-13 C6: optimistic state must reflect showLineNumbers: false',
        );
      },
    );

    test(
      'given_showLineNumbers_false_when_setShowLineNumbers_false_then_save_NOT_called_EC07',
      () async {
        // Arrange: default showLineNumbers == false.
        final repo = _RecordingRepository(initial: const AppSettings());
        final container = ProviderContainer(
        ...
        // Assert: save must NOT be called (EC-07 no-op write avoidance).
        expect(
          repo.savedArgs,
          isEmpty,
          reason:
              'EC-07: save must NOT be called when showLineNumbers is unchanged',
        );
      },
    );

    test(
      'given_settings_AsyncLoading_when_setShowLineNumbers_true_then_reads_defaults_and_does_not_throw_EC08',
      () async {
        // Arrange: trigger AsyncLoading by reading before the future settles.
        // const AppSettings() defaults showLineNumbers to false, so true differs
        // → save IS called once; no throw (EC-08).
        final repo = _RecordingRepository(initial: const AppSettings());
        final container = ProviderContainer(
          overrides: [settingsRepositoryProvider.overrideWithValue(repo)],
        );
        ...
        await expectLater(
          ...
          returnsNormally,
          reason:
              'EC-08: setShowLineNumbers must never throw during AsyncLoading '
              '(reads state.value ?? const AppSettings(), no requireValue)',
        );

        // repo.save must have been called once (false → true).
        expect(
          repo.savedArgs.length,
          equals(1),
          reason:
              'EC-08: save must be called once when AsyncLoading fallback '
              'gives showLineNumbers:false and new value is true',
        );
        expect(
          repo.savedArgs.first.showLineNumbers,
          isTrue,
          reason: 'EC-08: save must be called with showLineNumbers: true',
        );
      },
    );

    test(
      'given_showLineNumbers_false_when_setShowLineNumbers_true_then_state_is_AsyncData_true_synchronously_before_repo_save',
      () async {
        // Arrange: repo uses a completer-backed delay to verify the optimistic
        // update happens BEFORE the await.  We simulate this by checking state
        // inside a microtask-safe window: call the method, do NOT await it, then
        // immediately inspect the synchronous state.
        final repo = _RecordingRepository(initial: const AppSettings());
        final container = ProviderContainer(
          overrides: [settingsRepositoryProvider.overrideWithValue(repo)],
        );
        ...
        // Act: do NOT await — fire the future and inspect state synchronously.
        // ignore: unawaited_futures
        container.read(settingsProvider.notifier).setShowLineNumbers(true);

        // Assert synchronous optimistic update.
        final stateAfterCall = container.read(settingsProvider);
        expect(
          stateAfterCall.value?.showLineNumbers,
          isTrue,
          reason:
              'C6 optimistic: state.value.showLineNumbers must be true '
              'synchronously before repo.save resolves',
        );
      },
    );
  });
