...
void main() {
  group('SettingsScreen — structure', () {
    ...
    // -----------------------------------------------------------------------
    testWidgets('TC-6: no line-length / linelength row present', (
      tester,
    ) async {
      await _pumpSettingsScreen(tester, repo);

      // Assert no SwitchListTile title contains 'line' (case-insensitive).
      ...
      for (final sw in switches) {
        ...
        if (titleWidget is Text) {
          ...
          expect(
            text.contains('line'),
            isFalse,
            reason: 'Found a SwitchListTile with "line" in title: $text',
          );
        }
      }
    });
